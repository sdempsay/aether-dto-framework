...
- Skim **[AGENT-USAGE.md](AGENT-USAGE.md)** for current coordinates, packages, and consumer wiring
...
**Production and main-source code** (`aether-api`, `aether-builder-gen/src/main`, `aether-store-memory`, `aether-store-fs`, `aether-store-gen` processor/templates, generated builder templates):
...
- When `code-review` flags exceptional violations on store modules, fix them with exceptional utilities end-to-end; do not document carve-outs for internal I/O.
...
| Module | Role | Package root |
|--------|------|----------------|
| `aether-api` | **Contracts only** — annotations, builder SPI, store ports, failures, config SPI | `org.dempsay.aether.api.*` |
| `aether-builder-gen` | JSR-269: `*Builder` + `*Store` interfaces (compile-time only; not Felix install) | `org.dempsay.aether.processor` |
| `aether-runtime` | Maven aggregator (`aether-api` + exceptional); empty jar | — |
| `aether-store-memory` | In-memory provider (tests / light use) | `org.dempsay.aether.store.memory` |
| `aether-store-fs` | Filesystem JSON provider (Gson) + `FileStoreConfigService` | `org.dempsay.aether.store.fs` |
| `aether-store-gen` | `@AetherStoreProviders` + processor → Fs/Memory adapters (server/impl) | `org.dempsay.aether.store.gen` |

Reactor parent: `dempsay-felix-parent`. Public packages via `felix.bundle.exportcontents`. See [AGENT-USAGE.md](AGENT-USAGE.md) and `../dempsay-felix-parent/AGENT-USAGE.md`.

### Package space (do not invent flat packages)

**Public API (only under `org.dempsay.aether.api`):**

| Package | Contents |
|---------|----------|
| `…api.annotations` | `@AetherRecord`, `@Unique`, `@Singleton`, length/regex/`@Nullable` |
| `…api.access` | `AetherPrincipal` |
| `…api.builder` | `AetherBuilder` |
| `…api.failure` | `AetherFailure`, `AetherException`, `AetherResponses` |
| `…api.validation` | `ValidationException` |
| `…api.store` | `AetherResourceStore`, `AetherSingletonStore`, envelope types, `UpdateOptions`, abstract base |
| `…api.store.config` | `FileStoreConfig` SPI |
| `…api.store.unique` | Provider helpers: `UniqueConstraintModel`, `UniqueIndexTable`, `UniqueKey` (not app-facing CRUD) |

**Implementations (never under `api`):**

| Package | Module |
|---------|--------|
| `org.dempsay.aether.store.memory` | `aether-store-memory` |
| `org.dempsay.aether.store.fs` | `aether-store-fs` |
| `org.dempsay.aether.store.gen` | `aether-store-gen` |

**Do not** put store implementations in `aether-api`. **Do not** use legacy packages (`org.dempsay.aether.annotations`, `…store.AetherResourceStore` without `api`, `…store.config` without `api.store`).
...
- **Generated code must pass dempsay checkstyle** (templates in `aether-builder-gen` / `aether-store-gen`). Consumer projects may ignore generated sources; aether still emits clean Java. See PRD-updated “Generated code must pass checkstyle”.
- Run `mvn test` from the repo root to verify changes.
- **Testability goal:** depend on **API ports** + optional `aether-store-memory` so unit tests never need a live DB.
